...
|---|---|---|---|
| `renovate-operator` (renovate ns) | `ClusterRole/renovate-operator` grants `create/get/list/watch/update/delete` on **all** `secrets` cluster-wide | Renovate mints credential secrets per-job; needs to read its source secrets. Wildcard cluster-wide write feels broader than the workload calls for — verify whether it can be namespace-scoped to `renovate` only. | **Remediated** PR #11602 — `rbac.ownNamespaceOnly: true`; operator now uses Role + RoleBinding scoped to `renovate` ns. **Hardened further 2026-08-16** — chart `6.0.0`'s admission policy engine enabled (`policy.enabled: true`): outbound requests pinned to the default platform hosts, `requireSecretRefOptIn`, runner image restricted to approved `renovate/renovate` repos, non-root enforced. All three RenovateJobs verified to pass the defaults unchanged (each uses only `spec.secretRef` + `ghcr.io/renovatebot/renovate`). |
...
| `holmesgpt-view` (observability) | Built-in `view` cluster-wide | View tier reads almost everything except secrets. HolmesGPT is an LLM-fed pipeline; if the model context ever flows back to a less-trusted channel, this matters. | **Resolved** — `holmesgpt-view` binding and the companion `holmesgpt-extra-read` ClusterRole were deleted 2026-07-06 along with the HolmesGPT deployment (no longer applicable). |
